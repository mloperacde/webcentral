          ...
          >
            {stats.map((stat, index) => (
              <motion.div 
                ...
                className="relative p-6 border border-white/10 bg-white/[0.02] backdrop-blur-sm"
              >
                <div className="text-3xl sm:text-4xl font-light text-white mb-2">{stat.value}</div>
                <div className="text-[9px] text-white/40 tracking-[0.2em] uppercase font-bold">{t(stat.label)}</div>
              </motion.div>
            ))}
          </motion.div>
        </div>
